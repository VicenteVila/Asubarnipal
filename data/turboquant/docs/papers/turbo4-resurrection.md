...
- **@varjoranta** — [vibhavagarwal5/vllm PR #1](https://github.com/vibhavagarwal5/vllm/pull/1) (2026-04-02): Fixed vLLM TurboQuant default from 4-bit to FP8 values. Cites @TheTom (turbo4-resurrection) as source for "value precision is the quality bottleneck." Tested on A100 with Qwen3-8B — FP8 values match FP16 baseline, 4-bit produces garbage. @MidasMining confirmed 100% on 14-check reasoning benchmark with FP8. Independent vLLM validation that naive low-bit V quantization without rotation fails exactly as predicted.
- **@unamedkr / quantumaikr** — [quant.cpp](https://github.com/quantumaikr/quant.cpp) (2026-04-07): Literal end-to-end port of the TurboQuant paper (RHT → Lloyd-Max-Gaussian codebook → 1-bit QJL) in single-header C. QJL stage contributed byte-identical zero to attention scores regardless of input. Disabling it produced perplexity values bit-for-bit equal to the full pipeline. Provided theoretical explanation: the constant √(π/2)/m in the unbiased estimator is derived for Gaussian projection rows, and on Rademacher (±1) rows the per-key contribution gets dwarfed by the residual norm scaling. Independently took the freed bits and reinvested in 4-bit codebook (8 → 16 centroids), same path as our turbo4 resurrection. Llama 3.2 3B: turbo_kv_4b PPL 14.28 (+5.3% vs FP32), beating llama.cpp q4_0 KV (~14.99, +10.6%). Targets embedded/edge (iOS, Android, WASM, microcontrollers).
